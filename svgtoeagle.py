...
def drawSVG(svg_attributes, attributes, paths):
    ...
    while i < len(paths):
        ...
        if 'fill' in attributes[i].keys():
            filled = attributes[i].get('fill') != 'none' and attributes[i].get('fill') != ''
        ...
        if 'stroke' in attributes[i].keys():
            stroked = attributes[i].get('stroke') != 'none' and attributes[i].get('stroke') != ''
        elif 'style' in attributes[i].keys():
            stroked = style.get('stroke') != 'none' and style.get('stroke') != ''
        else:
            stroked = False
...
def convert(imagePath):
    ...
    if args.outMode != 'lib':
        ...
        paths, attributes, svg_attributes = string2paths(detectGroups(imagePath))
        ...
        paths, attributes, svg_attributes = string2paths(detectGroups(imagePath))
        scripts = [drawSVG(svg_attributes, attributes, paths)]
        ...
        try:
            ...
            if args.writeMode == 'a':
                new_contents = appendLib(scripts, ["importedimage"], output_path)
                ...
                f.write(writeLib(scripts, ["importedimage"]))
                f.close
...
def detectGroups(imagePath):

    tree = XMLET.parse(imagePath)
    root = tree.getroot()

    if len(root.findall(".//{*}g")) > 0:
        print("WARN: This SVG file contains groups and may not translate correctly")

    return XMLET.tostring(tree)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-u', dest='subSampling', default=2,
